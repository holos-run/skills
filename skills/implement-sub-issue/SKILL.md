...
Also move the ticket into the `In Progress` workflow state. Linear's native `In Progress` state signals lifecycle; the `role/*` label signals who owns the ticket.
